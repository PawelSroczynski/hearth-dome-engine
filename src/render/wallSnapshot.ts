...
import type { FloorSpec, RoofSpec } from '../core/building/model';
...
export function makeWallImages(wall: WallSpec, floor?: FloorSpec, roof?: RoofSpec): SpecImages {
  const renderer = new THREE.WebGLRenderer({ antialias: true, preserveDrawingBuffer: true });
  renderer.setPixelRatio(2);
  const model = buildingFromWall(wall, wall.depthMm ?? 4000, floor, roof);
  ...
  for (const r of wallBom([...allWallPanels(model), ...floorCassettes(model), ...roofPanels(model)])) {
